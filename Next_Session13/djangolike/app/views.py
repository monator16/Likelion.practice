...
def mypage(request):
    like_posts = Like.objects.filter(user=request.user)
    scrap_posts = Scrap.objects.filter(user=request.user)
    return render(request, 'mypage.html', { "like_posts":like_posts,"scrap_posts":scrap_posts  })
